    ...
    try:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            if result.get("total_amount") == expected.get("total_amount"):
                ...
                logger.warning(
                    f"  ✗ total_amount: {result.get('total_amount')} (expected {expected.get('total_amount')})"
                )
            ...
            if result.get("category_type") == expected.get("category_type"):
                ...
                logger.warning(
                    f"  ✗ category_type: {result.get('category_type')} (expected {expected.get('category_type')})"
                )
            ...
            if expected.get("category_name") in result.get("category_name", ""):
                ...
                logger.warning(
                    f"  ✗ category_name: {result.get('category_name')} (expected {expected.get('category_name')})"
                )
...
def test_image_url_processing():
    ...
    try:
        ...
        logger.info("  Testing image download and processing...")
        ...
        if result:
            ...
            logger.warning("  ⚠ Image processing returned None")
            logger.warning("  This could be due to network issues or URL blocking")
    ...
    for test_str, should_pass, description in test_cases:
        ...
        try:
            if cleaned_str:
                ...
                if not should_pass:
                    logger.info("    ✓ Correctly rejected empty string")
                    success_count += 1
                else:
                    logger.warning("    ✗ Should have parsed but got empty")
        except json.JSONDecodeError:
            if not should_pass:
                logger.info("    ✓ Correctly rejected non-JSON")
                success_count += 1
            else:
